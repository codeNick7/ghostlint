...
from tiramisu_cli.output import (
...
def _clone_repo(url: str) -> Path:
    ...
    tmp = tempfile.mkdtemp(prefix="tiramisu_")
...
def scan(
    ...
    Examples:
      ...
      tiramisu scan                                     # current directory

      tiramisu scan ~/myrepo                            # specific local path

      tiramisu scan --github owner/repo                 # clone & scan GitHub repo

      tiramisu scan --github https://github.com/o/r    # full URL

      tiramisu scan --headless                          # terminal-only, no browser

      tiramisu scan -e dead_code                        # single engine

      tiramisu scan --quick                             # fast engines only (pre-commit)

      tiramisu scan --format json                       # machine-readable for CI
    """
    # ── Resolve repo path ──────────────────────────────────────────────────────
...
def _run_scan(
    ...
    if changed:
        from tiramisu_engine.git_analyzer import GitAnalyzer
    ...
    if format != "json":
        import time
        from tiramisu_cli.html_report import generate_html_report
        from tiramisu_cli.web_server import prepare_report_dir, serve_and_open, cleanup_report_dir
...
def report() -> None:
    ...
    if not record:
        console.print("[yellow]No scans found. Run [bold]tiramisu scan[/bold] first.[/yellow]")
        raise typer.Exit(code=1)
...
def engines() -> None:  # noqa: F811 (this is the actual CLI command, not a shadowed name)
    ...
    from tiramisu_engine.detectors.registry import get_registry
    from rich.table import Table
    from rich import box
    ...
    console.print()
    console.print(t)
    console.print("[dim]Run a specific engine: tiramisu scan -e <engine>[/dim]\n")
...
mcp_app = typer.Typer(
    name="mcp",
    help="MCP (Model Context Protocol) server — connect tiramisu to AI coding tools.",
...
def mcp_callback(ctx: typer.Context) -> None:
    """Launch the tiramisu MCP stdio server, or run a subcommand (setup, info)."""
...
def _mcp_start() -> None:
    """Start the tiramisu MCP stdio server."""
    from tiramisu_mcp.server import mcp as mcp_server
...
def mcp_setup(
    ...
    """Auto-configure tiramisu as an MCP server for installed AI coding tools.

    Detects Claude Code, Cursor, Windsurf, and Zed and writes or patches
    their MCP config files. Existing tiramisu entries are updated in-place;
    other tools in the config are untouched.

    Examples:

      tiramisu mcp setup              # configure all detected tools

      tiramisu mcp setup --dry-run    # preview without writing

      tiramisu mcp setup --tool cursor  # configure Cursor only
    """
    from tiramisu_cli.mcp_setup import run_setup, print_manual_snippet
    from rich.table import Table
    from rich import box
    ...
    if not any_configured and not dry_run:
        ...
        console.print(
            ...
            "[dim]Verify with:[/dim] [bold]tiramisu mcp info[/bold]\n"
        )
...
def mcp_info() -> None:
    ...
    import sys
    from tiramisu_cli.mcp_setup import _server_entry, _tiramisu_on_path

    console.print("\n[bold]tiramisu MCP server[/bold]\n")

    entry = _server_entry()
    console.print(f"  command : [cyan]{entry['command']}[/cyan]")
    console.print(f"  args    : [cyan]{entry['args']}[/cyan]")
    if not _tiramisu_on_path():
        console.print(
            "  [yellow]Note: 'tiramisu' not found in PATH — using current Python interpreter.[/yellow]\n"
            "  [dim]Install globally with: pip install tiramisu  or  uv tool install tiramisu[/dim]"
        )
