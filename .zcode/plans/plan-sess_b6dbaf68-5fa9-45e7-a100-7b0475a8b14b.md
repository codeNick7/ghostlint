...
All Python runs use `/Users/vadirajdeshpande/workspace/ghostlint/app/.venv/bin/python`. Baseline (before): 1075 findings.

---

## Fix 1 — Orphan test detector (224 → ~2 findings)
**File:** `app/ghostlint_engine/detectors/test_health/detector.py`
...
**File:** `app/ghostlint_engine/detectors/refactor/detector.py`
...
**File:** `app/ghostlint_engine/detectors/dead_code/detector.py` (`_is_module_entry_point`, `_MODULE_ENTRY_POINT_STEMS`)
...
- Alembic `env.py` is already handled by the `env` ... actually add `env` stem OR keep it via the alembic `alembic/` dynamic segment (env.py sits at `app/alembic/env.py` — add `alembic` is already in dynamic segments, but env.py is at the alembic root, not versions/. Add `env` to stems).
...
**File:** `app/ghostlint_engine/detectors/doc_health/detector.py`
...
**File:** `app/ghostlint_engine/detectors/config_health/detector.py`
...
- *Required key missing (45)*: all 45 point at `.env.prod.template:1` — the "live" key resolution picks the template as `live_key` when no real `.env` is found at repo root (the real `app/.env` IS present but the matcher `k.endswith("/.env") or k == ".env"` should catch `app/.env` — verify; likely the issue is it compares against the template regardless).
...
**File:** `app/ghostlint_engine/detectors/naming/detector.py`
...
**Files:** `app/pyproject.toml` (add dep), `app/ghostlint_engine/ast_engine/ts_parser.py` (new), `app/ghostlint_engine/ast_engine/__init__.py` (register).
**Change:**
...
1. Re-run: `app/.venv/bin/python -m ghostlint_cli.main scan ~/workspace/COSMOS-Observation-Planner --format json > /tmp/baseline_after.json`
...
5. Update `app/report_analysis.md` with the before/after numbers.
