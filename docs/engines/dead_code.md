...
To suppress a finding, rename the function with a framework-recognized decorator or add it to the entry-point list in a future `tiramasu.yml` config.

## Running This Engine

```bash
tiramasu scan -e dead_code
tiramasu scan -e dead_code --min-confidence 0.75
tiramasu scan -e dead_code --changed       # only files changed vs HEAD
tiramasu scan --quick                      # dead_code is included in --quick
```
