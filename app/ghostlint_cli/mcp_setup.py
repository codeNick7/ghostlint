"""Auto-detect installed AI coding tools and write ghostlint MCP server configs."""
...
# Fallback when ghostlint isn't on PATH (e.g. installed in a venv)
_SERVER_ENTRY_PYTHON = {
    "command": sys.executable,
    "args": ["-m", "ghostlint_cli.main", "mcp"],
    "env": {},
}


def _ghostlint_on_path() -> bool:
    return shutil.which("ghostlint") is not None


def _server_entry() -> dict:
    if _ghostlint_on_path():
...
def _patch_config(tool_name: str, config_path: Path, entry: dict, dry_run: bool) -> str:
    """Merge the ghostlint MCP entry into *config_path*. Returns a status string."""
    ...
    if tool_name == "Zed":
        key = "context_servers"
        # Zed's schema differs: { "context_servers": { "ghostlint": { ... } } }
        servers = data.setdefault(key, {})
        existing = servers.get("ghostlint")
        if existing == entry:
            return "already configured — no changes needed"
        servers["ghostlint"] = entry
    else:
        key = "mcpServers"
        servers = data.setdefault(key, {})
        existing = servers.get("ghostlint")
    ...
    if dry_run:
        preview = json.dumps({key: {"ghostlint": entry}}, indent=2)
...
def run_setup(dry_run: bool = False, tool_filter: Optional[str] = None) -> list[tuple[str, str]]:
    """Auto-configure ghostlint MCP for all detected AI tools.
